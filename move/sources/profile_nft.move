...
public fun pay_for_generation(mut payment: Coin<SUI>, config: &PaymentConfig, ctx: &mut TxContext) {
...
public fun update_price(config: &mut PaymentConfig, new_price: u64, ctx: &mut TxContext) {
...
public fun update_treasury(config: &mut PaymentConfig, new_treasury: address, ctx: &mut TxContext) {
    assert!(ctx.sender() == config.treasury, EInvalidPrice);
